...
`node/RECOVERY.md`). When the API is down CI cannot reach anything — work from
...
- **Bad `config.yaml`.** Written by `k3s-cluster/`'s `k3s_server` role. Move it
...
letting `k3s-cluster/site.yml` reinstall it clean, restore etcd from the
...
LAN-local, as `node/RECOVERY.md` describes.
