...
the network config into `/etc/uci-defaults` scripts that apply on first boot. It
is built and flashed by hand — CI does not touch it.
...
- root/LuCI password unchanged from the current router, written from the hash in
  the GL.iNet backup. The Ansible playbook does not manage it.
